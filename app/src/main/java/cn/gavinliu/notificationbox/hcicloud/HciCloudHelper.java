...
public class HciCloudHelper {
    private static final String TAG ="HciCloudHelper";
	 ...
	 * @return
	 */
	public static byte[] getAssetFileData(String fileName) {
		InputStream in = null;
		int size = 0;
		try {
	...
	public static byte[] getAssetFileDataLines(String fileName) {
		InputStream in = null;
		int size = 0;
		try {
			in = mContext.getResources().getAssets().open(fileName);
...
//				copyAssetFile(assetMgr, "data" + File.separator	+ string, dataPath + File.separator + string);
//			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static void LoopFiles(String []filesList, AssetManager assetMgr, String srcPath, String destPath)
	{
		for(String string : filesList)
		{
			...
			try {
				...
				LoopFiles(file, assetMgr, srcPath + string +File.separator, destPath + string +File.separator);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
